...
//    //MARK: - JSON Parsing
    func updatePriceData(json : JSON) {
        ...
        if let priceResult = json["ask"].double {
            updateUI(price: String(priceResult), row: currencyPicker.selectedRow(inComponent: 0))
        }
        else {
    ...
    //MARK: UI Update
    func updateUI(price : String, row : Int) {
        bitcoinPriceLabel.text = symbolArray[row]+String(price)
    }
